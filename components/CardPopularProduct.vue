<template>
  <div v-if="showSlice" class="mb-10 padding-card bg-lightgrey">
    <ContentMaxWidth>
      <div class="">
        <div class="mb-heading gutter-special flex flex-col justify-center">
          <nuxt-img v-if="image.url" :src="image.url" :alt="image.alt" class="h-[200px] lg:h-80 w-full max-w-md object-cover rounded-lg" />
        </div>
        <div class="gutter">
          <h4 class="mb-2 text-teal typo-h-highlight lg:typo-h-highlight-desktop">
            {{ $prismic.asText(highlighted) }}
          </h4>
...
export default {
  ...
  computed: {
    ...
    highlighted() {
      return this.data?.highlighted_heading
    },
    image() {
